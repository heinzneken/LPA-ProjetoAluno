    ...
    def run(self):
        ...
        while True:
            ...
            for event in pygame.event.get():
                # rotina para executar a função quit na janela.
                # e a rotina sys.exit() serve para ocultar os erros e advertencias geradas qdo encerrado
                if event.type == pygame.QUIT:
                    pygame.quit()
                    sys.exit()
                if event.type == pygame.KEYDOWN: # testar se alguma tecla foi pressionada
                    if event.key == pygame.K_DOWN: # testar se a tecla seta para baixo foi pressionada
                        if menu_option < len(MENU_OPTION) - 1:
                            menu_option += 1
                        else:
                            menu_option = 0

                    if event.key == pygame.K_UP: # testar se a tecla seta para baixo foi pressionada
                        if menu_option > 0:
                            menu_option -= 1
                        else:
